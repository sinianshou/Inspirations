...
-(instancetype)initWithCenterButtonRadius:(CGFloat)CBRadius{
    self = [self init];
    if (self) {
        [self setCenterButtonWithRadius:CBRadius];
    }
    return self;
}
-(void)setDefault
...
{
    if (_shouldClickCenterButton != nil && !_shouldClickCenterButton()) {
        return;
    }
...
- (void)animationDidStop:(CAAnimation *)theAnimation finished:(BOOL)flag
{
    ...
    if (_clickCenterButtonFinished != nil) {
        _shouldClickCenterButton();
    }
}
//按钮事件
...
-(CGRect)circleMenuExpandRect{
    if (CGRectEqualToRect(_circleMenuExpandRect, CGRectZero) || CGRectEqualToRect(_circleMenuExpandRect, CGRectNull)) {
        if (self.circleButtons.count > 0) {
            CGFloat length = self.circleButtonsCenterrRaduis*2 + self.circleButtons[0].bounds.size.height + 2*3;
...
-(CGFloat)circleButtonsCenterrRaduis{
    if (_circleButtonsCenterrRaduis <= 0 && self.circleButtons.count > 0) {
